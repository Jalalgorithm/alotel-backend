...
/** One country in the discounts grid. Selecting it loads the edit panel; configured countries can be deleted directly. */
export const DiscountCard = ({ country, rule, isSelected, isDeleting, onSelect, onDelete }) => (
  <div
    className={cn(
  ...
  >
    <button type="button" onClick={() => onSelect(country)} aria-pressed={isSelected} className="flex w-full flex-col gap-2 text-left">
      <div className="flex items-start justify-between gap-2 pr-6">
        ...
        <Badge variant={rule ? (rule.isActive ? 'ok' : 'neutral') : 'neutral'}>
          {rule ? (rule.isActive ? 'Active' : 'Paused') : 'Not set'}
        </Badge>
      </div>
      ...
      {rule ? (
        ...
        <p className="text-[11px] text-ink-muted">No promotional discount configured.</p>
      )}
    </button>
